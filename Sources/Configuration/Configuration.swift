...
extension ConfigurationData {
    ...
    static func read(_ type: ConfigurationData.Type, fromFile jsonFile: FilePath) throws -> ConfigurationData {
        guard let fileURL = URL(filePath: jsonFile) else {
    ...
    static func write(_ config: ConfigurationData, toFile jsonFile: FilePath) async throws {
        guard let fileURL = URL(filePath: jsonFile) else {
            throw URLError(.badURL)
        }
        
        let data = try JSONEncoder().encode(config)
        if FileManager.default.fileExists(atPath: jsonFile.string) {
